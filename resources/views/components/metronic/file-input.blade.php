...
<div class="row gx-1">
    ...
    <div class="col-2 mt-n7">
        <img id="{{ $id ?? 'file-input' }}-preview" src="{{ !empty($source) ? $source : asset('images/no_image.png') }}"
            alt="Image Preview" class="img-thumbnail file-preview" style="display: {{ !empty($source) ? 'block' : 'none' }};" width="55px"
            height="68px">
    </div>
</div>
...
<script>
    ...
    function previewFile(input) {
        var file = input.files[0];
        var preview = input.closest('.row').querySelector('.file-preview'); // Using closest to find the parent .row and querySelector for .file-preview
        var reader = new FileReader();
